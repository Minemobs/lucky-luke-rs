pub mod bl {
    ...
    pub async fn treat(ctx: &Context, msg: &Message) -> Result<(), SerenityError> {
        ...
        //Sends to message's author an embed
        //(that's fucking ugly I know btw)
        match msg.author.dm(
            &ctx.http,
                ...
                message.embed(
                    |embed| {
                        ...
                        embed.author(
                            |author| {
                                author.name(
                                    "Lucky Luke - Le balayeur de skids."
                                )
                            });
                        ...
                        embed.description(
                            format!(
                                "Il semblerait que tu sois un skid...\nJe t'ai entendu dire :\n```\n{}\n```\nEt mon devoir est de balayer ton message ! :wind_blowing_face:",
                                msg.content
                            )
                        );
    ...
    pub fn is_blacklisted(msg: &Message) -> Option<bool> {
        ...
        for blacklisted_word in blacklisted_words.split_whitespace().into_iter() {
            if content.to_lowercase().contains(blacklisted_word) {
                return Some(true)
            }
        }
